...
9. **Knowledge Capture (Update Memory Log):**
   ...
   - **Commit Memory:** Stage and commit the memory update with `docs(memory): Record insights from task '<Task Name>'`.
...
2.  **Knowledge Synthesis (Update Brain):**
    ...
    -   **Commit:** Stage and commit the consolidated memory with `docs(memory): Consolidate brain for phase '<Phase Name>'`.
